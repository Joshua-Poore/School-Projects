...
//    terrain_height_field->ping_pong_strength = 7;
    ...
    artfc->the_shape = c;
    artfc->world_transform.scale = glm::vec3(2, 4, 2);
    artfc->world_transform.translate = glm::vec3(50, ((terrain_height_field->altitude(50,25))+2), 25);

    artf_list.push_back(artfc);
    //Cylinder
    cs4722::shape* d = new cs4722::cylinder;

    auto* artfd = new cs4722::artifact();
    artfd->the_shape = d;
    artfd->world_transform.scale = glm::vec3(2, 4, 2);
    artfd->world_transform.translate = glm::vec3(25, ((terrain_height_field->altitude(25,50))+2), 50);

    artf_list.push_back(artfd);
    //Sphere
    cs4722::shape* e = new cs4722::sphere;

    auto* artfe = new cs4722::artifact();
    artfe->the_shape = e;
    artfe->world_transform.scale = glm::vec3(2, 4, 2);
    artfe->world_transform.translate = glm::vec3(25, ((terrain_height_field->altitude(25,25))+4), 25);

    artf_list.push_back(artfe);
    //Torus
    cs4722::shape* f = new cs4722::torus;

    auto* artff = new cs4722::artifact();
    artff->the_shape = f;
    artff->world_transform.scale = glm::vec3(2, 4, 2);
    artff->world_transform.translate = glm::vec3(50, ((terrain_height_field->altitude(50,50))+2), 50);

    artf_list.push_back(artff);
...
void general_key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
    ...
    */
    ...
    if(movement_restricted) {
        ...
        }else if (the_view->camera_position.x < 0){
            the_view->camera_position = position0;
        }else if(the_view->camera_position.x > (terrain_scale.x +terrain_translate.x)){
            the_view->camera_position = position0;
        }else if(the_view->camera_position.z < 0){
            the_view->camera_position = position0;
        }else if(the_view->camera_position.z > (terrain_scale.z +terrain_translate.z)){
            the_view->camera_position = position0;
        }
    }
#ifdef LOG_FLUP
